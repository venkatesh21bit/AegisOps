...
from aegisops.memory.episodic_memory import EpisodicMemoryManager
from aegisops.memory.semantic_memory import retrieve_runbook
from langchain_core.messages import SystemMessage
from langchain_core.prompts import ChatPromptTemplate, MessagesPlaceholder
from aegisops.agent.tools_config import tools_list
from aegisops.integrations.itsm_client import close_itsm_ticket
from aegisops.integrations.slack_client import send_resolution_summary
...
# Switched to Gemini per user request
...
1. Observe the system telemetry using diagnostic tools (get_logs, get_pod_status, splunk_search_logs, splunk_get_error_rate).
...
You have access to Splunk via secured MCP tools (routed through the Semantic MCP Firewall):
- splunk_search_logs: Search service logs with SPL syntax, filtered by log level and time range.
- splunk_get_error_rate: Query HTTP 5xx error rate metrics over a configurable window.
- splunk_get_service_info: Retrieve aggregated service metadata (event counts, hosts, sources).
All MCP tool calls are evaluated by the firewall before execution. Dangerous SPL patterns (delete, rest, outputcsv) will be blocked.

--- COST CONTROL: OPENTELEMETRY INGESTION PURIFIER ---
During error storms, use the OTel tools to reduce log ingestion costs at the collector level:
- otel_analyze_log_noise: Identify repetitive noise vs. actionable signals in log streams.
- otel_generate_filter_rules: Generate deployable OTel Collector filter YAML configs with cost estimates.
- otel_apply_filter_configmap: Stage a Kubernetes ConfigMap update for the OTel Collector (requires HITL approval).
Always analyze noise patterns BEFORE generating filter rules. Never suppress ERROR or FATAL severity logs.
"""
